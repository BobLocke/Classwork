...
float evalExpr( Expr *expr )
{
    switch(expr->type)
    {
        ...
        case VAR_EXPR:
           // printf("%f\n", expr->op.sym->val);
            return expr->op.sym->val;                                                                   /// SYMBOL STUFF
        case ADD_EXPR:
...
void executeStmt(Stmt *stmt_)
{
    ...
    switch(stmt_->type){
        ...
        case BLOCK_STMT:
            ...
            break;           
        case ASSIGN_STMT:
            stmt_->s.assign_.lval->val = evalExpr(stmt_->s.assign_.rval);
            break;
        default:
           break; 
    }
...
Stmt* assign_stmt(){
    Stmt *s = (Stmt*) malloc(sizeof(Stmt));
    s->type = ASSIGN_STMT;
    s->s.assign_.lval = symLookup(lval->s);
    match(IDENT_);
    match(ASSIGN_);
...
Stmt* ifBlock(){
    ...
    s->type = BLOCK_STMT;
    s->s.block_.list = stmt();
    while(lookahead != FI_ && lookahead != ELSE_ && lookahead != ELSIF_){
        s->s.block_.list->next = ifBlock();
    }
    return s;
}
